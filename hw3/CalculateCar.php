...
<body>
    ...
    function DownPayment($arr_mc,$price_down,&$total){
        global $move;
        $total = $price_down;
        if($move == 0){
     ...
     function installment($arr_mc , &$total, &$install){
        $install = $arr_mc - $total;
        return $install;
    }

    function interestYear($arr_interyear,&$install,&$interyear){
        $interyear = $install * $arr_interyear;
        return $interyear;
        
    }
    
    function interestTotal($term,&$interyear,&$intertotal){
        $intertotal = $interyear * ($term / 12);
        return $intertotal;

    }

    function payMonth($term,&$install,&$intertotal){
